...
class Ball():
    ...
    def __init__(self, screen, startpoint=Vec2d(5,5), move=Vec2d(1,0), radius = 15, color=(0,0,255), bossnumber=0, shape="circle", linewidth=1, linelenght=15, damage=1000):
...
class PygView(object):
    ...
    def paint(self):
        ...
        self.niklas_ship = Shape(self.screen, Vec2d(400, 200),
                                       (
                                        ...
                                        Vec2d(40, -10),
                                        Vec2d(-40, -40),
                                        Vec2d(0, 0)),color=(255,0,0), cooldowntime=1)
    ...
    def run(self):
        ...
        while running:
            ...
            text_player3 = "Player3: {}° {} hp".format(self.niklas_ship.angle % 360, self.niklas_ship.hitpoints)
            self.draw_text(text_player1, x=50, y=30, color=(0,200,0))
            self.draw_text(text_time, x = self.width//2-200, y=70, color=(100,0,100))
            self.draw_text(text_player3, x = self.width//2-200, y=30, color=(200,0,0))
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_r:
                        if self.niklas_ship.cooldown > self.niklas_ship.cooldowntime:
                            ...
                            bcolor=(10, random.randint(200,247), 70)
                            for k in range(100):
                                #move = Vec2d(1, 0).rotated(k*8)*3
                                move = Vec2d(1, 0).rotated(random.randint(0,500))*5
                                Ball(self.screen, self.niklas_ship.startpoint+ move*-10, move*30,
                                    bossnumber=self.niklas_ship.number, color = bcolor, shape="circle", radius=5, damage=10000000)
            ...
            for b in Ball.group:
                ...
                if b.bossnumber != self.yanniks_ship.number:
                    if (b.startpoint - self.yanniks_ship.startpoint).get_length() < critical_distance:
                        ...
                        b.age = Ball.maxage
                if b.bossnumber != self.niklas_ship.number:
